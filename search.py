...
Version: 3.2 (OpenRouter - Works in Russia)
"""
...
class OpenRouterProvider(LLMProvider):
    """OpenRouter provider (FREE, works in Russia!)"""
    ...
    async def get_answer(self, question: str, options: List[str]) -> Tuple[str, str]:
        ...
        headers = {
            ...
            "HTTP-Referer": "https://github.com/medical-test-automation",
        ...
        data = {
            ...
            "messages": [
                {
                    "role": "system",
                    "content": "You are a medical expert. Answer with the number (1-4) and brief explanation."
                },
                {
                    "role": "user",
                    "content": prompt
        ...
        try:
            ...
            async with aiohttp.ClientSession(timeout=timeout) as session:
                async with session.post(
                    ...
                    if response.status != 200:
                        ...
                        console.print(f"[red]OpenRouter Error {response.status}: {response_text[:300]}[/red]")
            ...
            console.print(f"[red]OpenRouter Error: {str(e)[:300]}[/red]")
            raise
        ...
        try:
            ...
            async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
                async with session.post(
                    ...
                    if response.status != 200:
                        console.print(f"[red]Status: {response.status}[/red]")
                        console.print(f"[red]Response: {response_text[:500]}[/red]")
                        raise Exception(f"Groq API error ({response.status}): {response_text[:200]}")
            ...
            console.print(f"[red]Groq Error: {str(e)[:300]}[/red]")
            raise
...
class MedicalTestAutomation:
    ...
    def _initialize_llm(self):
        ...
        if provider == "openrouter":
            ...
            self.llm_provider = GroqProvider(api_key, model)
            console.print(f"[green]✓ Groq initialized with model: {model}[/green]")
    ...
    async def setup_browser(self):
        ...
        # Launch browser with proper display settings
        self.browser = await self.playwright.chromium.launch(
            ...
            args=[
                ...
                '--force-device-scale-factor=1',  # IMPORTANT: Fix scaling/zoom issues
                '--high-dpi-support=1',
        ...
        # 1280x800 is a safe size that works on most displays
        viewport_width = 1280
        viewport_height = 800
        ...
        context = await self.browser.new_context(
            ...
            device_scale_factor=1,  # IMPORTANT: Prevents zoom issues
            is_mobile=False,
            has_touch=False,
        )
...
Examples:
  ...
  python main.py --debug            # Save screenshots
    ...
    console.print(Panel.fit(
        ...
        "[dim]v3.2 - OpenRouter (Works in Russia!)[/dim]",
        border_style="cyan"
    ))
    console.print("\n")
